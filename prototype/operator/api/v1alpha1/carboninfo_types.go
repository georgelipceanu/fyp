...
type CarbonInfoSpec struct {
	...
	// foo is an example field of CarbonInfo. Edit carboninfo_types.go to remove/update
	// +optional
...
type CarbonInfoStatus struct {
	...
	// conditions represent the current state of the CarbonInfo resource.
...
type CarbonInfo struct {
	...
	// spec defines the desired state of CarbonInfo
	// +required
	Spec CarbonInfoSpec `json:"spec"`

	// status defines the observed state of CarbonInfo
	// +optional
	Status CarbonInfoStatus `json:"status,omitempty,omitzero"`
}
